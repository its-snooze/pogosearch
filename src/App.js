...
import { 
  ...
  Bookmark, BookmarkCheck, Trash2, Download, Upload, Sun, Moon, Globe, MessageCircle, Trophy
} from 'lucide-react';
...
import raidBossesData from './data/raidBosses.json';
...
const PokemonGoSearchBuilder = () => {
  ...
  const [expandedRaidTiers, setExpandedRaidTiers] = useState({
    tier5: false,
    mega: false,
    tier3: false,
    tier1: false,
  ...
  const toggleRaidTier = (tier) => {
    setExpandedRaidTiers(prev => ({
      ...prev,
      [tier]: !prev[tier]
    }));
  };
  ...
  // Copy counters to clipboard with translation
  const copyCounters = useCallback(async (counters, event) => {
    try {
      // Translate each counter name to the current language
      const translatedCounters = counters.map(counter => {
        if (selectedLanguage === 'English') {
          return counter.toLowerCase();
        }
        const translated = translateTerm(counter, selectedLanguage, 'name');
        return translated.toLowerCase();
      });

      // Generate search string with comma-separated Pokemon names
      const searchString = translatedCounters.join(',');
      ...
      // Set toast message
      setToastMessage(getUIText('counters_copied', selectedLanguage));
      ...
      console.error('Failed to copy counters:', err);
    }
  }, [selectedLanguage]);
  ...
  const applyPremadeSearch = async (searchKey, event) => {
    ...
    if (preset) {
      ...
      try {
        ...
        // Set toast message
        setToastMessage(getUIText('copied', selectedLanguage));
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-cyan-50/50 text-slate-900 dark:from-slate-950 dark:via-slate-950 dark:to-slate-900 dark:text-slate-100 transition-colors duration-300">
      ...
      <div className="w-full max-w-full md:max-w-7xl md:mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-6 sm:py-8">
        ...
        {/* Current Raid Bosses Section */}
        <div className="mb-6">
          <div className="bg-gradient-to-r from-amber-500 to-orange-500 rounded-xl shadow-lg overflow-hidden border-2 border-amber-400 dark:border-amber-600">
            <div className="p-4 sm:p-5">
              <div className="flex items-center gap-3 mb-2">
                <Trophy className="w-6 h-6 sm:w-7 sm:h-7 text-white flex-shrink-0" />
                <h2 className="text-xl sm:text-2xl font-bold text-white">
                  {getUIText('current_raid_bosses', selectedLanguage)}
                </h2>
              </div>
              <p className="text-sm sm:text-base text-amber-50 mb-4">
                {getUIText('last_updated', selectedLanguage)}: {raidBossesData.lastUpdated} | {raidBossesData.eventName} ({raidBossesData.eventDates})
              </p>

              {/* Raid Tiers */}
              <div className="space-y-3">
                {/* Tier 5 */}
                {raidBossesData.tier5 && raidBossesData.tier5.length > 0 && (
                  <div className="bg-white/90 backdrop-blur-sm rounded-lg shadow-md overflow-hidden border border-amber-200 dark:bg-slate-900/90 dark:border-amber-600/50">
                    <button
                      onClick={() => toggleRaidTier('tier5')}
                      className="w-full min-h-[48px] px-4 flex items-center justify-between bg-gradient-to-r from-purple-600 to-indigo-600 text-white hover:opacity-95 transition-all duration-200"
                    >
                      <span className="font-bold text-sm sm:text-base">{getUIText('tier_5_raids', selectedLanguage)}</span>
                      <ChevronDown className={`w-5 h-5 transition-transform duration-300 ${expandedRaidTiers.tier5 ? 'rotate-180' : ''}`} />
                    </button>
                    {expandedRaidTiers.tier5 && (
                      <div className="p-4 space-y-3 category-expand">
                        {raidBossesData.tier5.map((boss, idx) => (
                          <div key={idx} className="bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-slate-800 dark:to-slate-800 rounded-lg p-4 border border-purple-200 dark:border-purple-700">
                            <div className="flex items-start justify-between gap-3 mb-2">
                              <div className="flex-1">
                                <h3 className="font-bold text-base sm:text-lg text-gray-900 dark:text-white mb-2">{boss.name}</h3>
                                <div className="flex flex-wrap gap-2 mb-3">
                                  {boss.types.map((type, typeIdx) => (
                                    <span
                                      key={typeIdx}
                                      className={`px-2 py-1 rounded-md text-xs font-semibold ${typeColors[type.toLowerCase()] || 'bg-gray-500 text-white'}`}
                                    >
                                      {type}
                                    </span>
                                  ))}
                            ...
                            <button
                              onClick={(e) => copyCounters(boss.counters, e)}
                              className="w-full px-4 py-2 bg-gradient-to-r from-purple-500 to-indigo-500 hover:from-purple-600 hover:to-indigo-600 text-white rounded-lg font-semibold text-sm transition-all duration-200 hover:shadow-lg active:scale-95 flex items-center justify-center gap-2"
                            >
                              <Copy className="w-4 h-4" />
                              {getUIText('copy_counters', selectedLanguage)}
                            </button>
                          </div>
                        ))}
                      </div>
                ...
                {/* Mega Raids */}
                {raidBossesData.mega && raidBossesData.mega.length > 0 && (
                  <div className="bg-white/90 backdrop-blur-sm rounded-lg shadow-md overflow-hidden border border-amber-200 dark:bg-slate-900/90 dark:border-amber-600/50">
                    <button
                      onClick={() => toggleRaidTier('mega')}
                      className="w-full min-h-[48px] px-4 flex items-center justify-between bg-gradient-to-r from-pink-600 to-rose-600 text-white hover:opacity-95 transition-all duration-200"
                    >
                      <span className="font-bold text-sm sm:text-base">{getUIText('mega_raids', selectedLanguage)}</span>
                      <ChevronDown className={`w-5 h-5 transition-transform duration-300 ${expandedRaidTiers.mega ? 'rotate-180' : ''}`} />
                    </button>
                    {expandedRaidTiers.mega && (
                      <div className="p-4 space-y-3 category-expand">
                        {raidBossesData.mega.map((boss, idx) => (
                          <div key={idx} className="bg-gradient-to-r from-pink-50 to-rose-50 dark:from-slate-800 dark:to-slate-800 rounded-lg p-4 border border-pink-200 dark:border-pink-700">
                            <div className="flex items-start justify-between gap-3 mb-2">
                              <div className="flex-1">
                                <h3 className="font-bold text-base sm:text-lg text-gray-900 dark:text-white mb-2">{boss.name}</h3>
                                <div className="flex flex-wrap gap-2 mb-3">
                            ...
                            <button
                              onClick={(e) => copyCounters(boss.counters, e)}
                              className="w-full px-4 py-2 bg-gradient-to-r from-pink-500 to-rose-500 hover:from-pink-600 hover:to-rose-600 text-white rounded-lg font-semibold text-sm transition-all duration-200 hover:shadow-lg active:scale-95 flex items-center justify-center gap-2"
                            >
                              <Copy className="w-4 h-4" />
                              {getUIText('copy_counters', selectedLanguage)}
                            </button>
                ...
                {/* Tier 3 */}
                {raidBossesData.tier3 && raidBossesData.tier3.length > 0 && (
                  <div className="bg-white/90 backdrop-blur-sm rounded-lg shadow-md overflow-hidden border border-amber-200 dark:bg-slate-900/90 dark:border-amber-600/50">
                    <button
                      onClick={() => toggleRaidTier('tier3')}
                      className="w-full min-h-[48px] px-4 flex items-center justify-between bg-gradient-to-r from-blue-600 to-cyan-600 text-white hover:opacity-95 transition-all duration-200"
                    >
                      <span className="font-bold text-sm sm:text-base">{getUIText('tier_3_raids', selectedLanguage)}</span>
                      <ChevronDown className={`w-5 h-5 transition-transform duration-300 ${expandedRaidTiers.tier3 ? 'rotate-180' : ''}`} />
                    </button>
                    {expandedRaidTiers.tier3 && (
                      <div className="p-4 space-y-3 category-expand">
                        {raidBossesData.tier3.map((boss, idx) => (
                          <div key={idx} className="bg-gradient-to-r from-blue-50 to-cyan-50 dark:from-slate-800 dark:to-slate-800 rounded-lg p-4 border border-blue-200 dark:border-blue-700">
                            <div className="flex items-start justify-between gap-3 mb-2">
                              <div className="flex-1">
                                <h3 className="font-bold text-base sm:text-lg text-gray-900 dark:text-white mb-2">{boss.name}</h3>
                                <div className="flex flex-wrap gap-2 mb-3">
                            ...
                            <button
                              onClick={(e) => copyCounters(boss.counters, e)}
                              className="w-full px-4 py-2 bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-white rounded-lg font-semibold text-sm transition-all duration-200 hover:shadow-lg active:scale-95 flex items-center justify-center gap-2"
                            >
                              <Copy className="w-4 h-4" />
                              {getUIText('copy_counters', selectedLanguage)}
                            </button>
                ...
                {/* Tier 1 */}
                {raidBossesData.tier1 && raidBossesData.tier1.length > 0 && (
                  <div className="bg-white/90 backdrop-blur-sm rounded-lg shadow-md overflow-hidden border border-amber-200 dark:bg-slate-900/90 dark:border-amber-600/50">
                    <button
                      onClick={() => toggleRaidTier('tier1')}
                      className="w-full min-h-[48px] px-4 flex items-center justify-between bg-gradient-to-r from-green-600 to-emerald-600 text-white hover:opacity-95 transition-all duration-200"
                    >
                      <span className="font-bold text-sm sm:text-base">{getUIText('tier_1_raids', selectedLanguage)}</span>
                      <ChevronDown className={`w-5 h-5 transition-transform duration-300 ${expandedRaidTiers.tier1 ? 'rotate-180' : ''}`} />
                    </button>
                    {expandedRaidTiers.tier1 && (
                      <div className="p-4 space-y-3 category-expand">
                        {raidBossesData.tier1.map((boss, idx) => (
                          <div key={idx} className="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-slate-800 dark:to-slate-800 rounded-lg p-4 border border-green-200 dark:border-green-700">
                            <div className="flex items-start justify-between gap-3 mb-2">
                              <div className="flex-1">
                                <h3 className="font-bold text-base sm:text-lg text-gray-900 dark:text-white mb-2">{boss.name}</h3>
                                <div className="flex flex-wrap gap-2 mb-3">
                            ...
                            <button
                              onClick={(e) => copyCounters(boss.counters, e)}
                              className="w-full px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white rounded-lg font-semibold text-sm transition-all duration-200 hover:shadow-lg active:scale-95 flex items-center justify-center gap-2"
                            >
                              <Copy className="w-4 h-4" />
                              {getUIText('copy_counters', selectedLanguage)}
                            </button>
                ...
                {/* Shadow Tier 5 */}
                {raidBossesData.shadow5 && raidBossesData.shadow5.length > 0 && (
                  <div className="bg-white/90 backdrop-blur-sm rounded-lg shadow-md overflow-hidden border border-amber-200 dark:bg-slate-900/90 dark:border-amber-600/50">
                    <button
                      onClick={() => toggleRaidTier('shadow5')}
                      className="w-full min-h-[48px] px-4 flex items-center justify-between bg-gradient-to-r from-red-700 to-red-900 text-white hover:opacity-95 transition-all duration-200"
                    >
                      <span className="font-bold text-sm sm:text-base">{getUIText('shadow_5_raids', selectedLanguage)}</span>
                      <ChevronDown className={`w-5 h-5 transition-transform duration-300 ${expandedRaidTiers.shadow5 ? 'rotate-180' : ''}`} />
                    </button>
                    {expandedRaidTiers.shadow5 && (
                      <div className="p-4 space-y-3 category-expand">
                        {raidBossesData.shadow5.map((boss, idx) => (
                          <div key={idx} className="bg-gradient-to-r from-red-50 to-rose-50 dark:from-slate-800 dark:to-slate-800 rounded-lg p-4 border border-red-200 dark:border-red-700">
                            <div className="flex items-start justify-between gap-3 mb-2">
                              <div className="flex-1">
                                <h3 className="font-bold text-base sm:text-lg text-gray-900 dark:text-white mb-2">{boss.name}</h3>
                                <div className="flex flex-wrap gap-2 mb-3">
                            ...
                            <button
                              onClick={(e) => copyCounters(boss.counters, e)}
                              className="w-full px-4 py-2 bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 text-white rounded-lg font-semibold text-sm transition-all duration-200 hover:shadow-lg active:scale-95 flex items-center justify-center gap-2"
                            >
                              <Copy className="w-4 h-4" />
                              {getUIText('copy_counters', selectedLanguage)}
                            </button>
                ...
                {/* Shadow Tier 3 */}
                {raidBossesData.shadow3 && raidBossesData.shadow3.length > 0 && (
                  <div className="bg-white/90 backdrop-blur-sm rounded-lg shadow-md overflow-hidden border border-amber-200 dark:bg-slate-900/90 dark:border-amber-600/50">
                    <button
                      onClick={() => toggleRaidTier('shadow3')}
                      className="w-full min-h-[48px] px-4 flex items-center justify-between bg-gradient-to-r from-orange-700 to-red-700 text-white hover:opacity-95 transition-all duration-200"
                    >
                      <span className="font-bold text-sm sm:text-base">{getUIText('shadow_3_raids', selectedLanguage)}</span>
                      <ChevronDown className={`w-5 h-5 transition-transform duration-300 ${expandedRaidTiers.shadow3 ? 'rotate-180' : ''}`} />
                    </button>
                    {expandedRaidTiers.shadow3 && (
                      <div className="p-4 space-y-3 category-expand">
                        {raidBossesData.shadow3.map((boss, idx) => (
                          <div key={idx} className="bg-gradient-to-r from-orange-50 to-red-50 dark:from-slate-800 dark:to-slate-800 rounded-lg p-4 border border-orange-200 dark:border-orange-700">
                            <div className="flex items-start justify-between gap-3 mb-2">
                              <div className="flex-1">
                                <h3 className="font-bold text-base sm:text-lg text-gray-900 dark:text-white mb-2">{boss.name}</h3>
                                <div className="flex flex-wrap gap-2 mb-3">
                            ...
                            <button
                              onClick={(e) => copyCounters(boss.counters, e)}
                              className="w-full px-4 py-2 bg-gradient-to-r from-orange-600 to-red-600 hover:from-orange-700 hover:to-red-700 text-white rounded-lg font-semibold text-sm transition-all duration-200 hover:shadow-lg active:scale-95 flex items-center justify-center gap-2"
                            >
                              <Copy className="w-4 h-4" />
                              {getUIText('copy_counters', selectedLanguage)}
                            </button>
                ...
                {/* Shadow Tier 1 */}
                {raidBossesData.shadow1 && raidBossesData.shadow1.length > 0 && (
                  <div className="bg-white/90 backdrop-blur-sm rounded-lg shadow-md overflow-hidden border border-amber-200 dark:bg-slate-900/90 dark:border-amber-600/50">
                    <button
                      onClick={() => toggleRaidTier('shadow1')}
                      className="w-full min-h-[48px] px-4 flex items-center justify-between bg-gradient-to-r from-gray-700 to-slate-800 text-white hover:opacity-95 transition-all duration-200"
                    >
                      <span className="font-bold text-sm sm:text-base">{getUIText('shadow_1_raids', selectedLanguage)}</span>
                      <ChevronDown className={`w-5 h-5 transition-transform duration-300 ${expandedRaidTiers.shadow1 ? 'rotate-180' : ''}`} />
                    </button>
                    {expandedRaidTiers.shadow1 && (
                      <div className="p-4 space-y-3 category-expand">
                        {raidBossesData.shadow1.map((boss, idx) => (
                          <div key={idx} className="bg-gradient-to-r from-gray-50 to-slate-50 dark:from-slate-800 dark:to-slate-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
                            <div className="flex items-start justify-between gap-3 mb-2">
                              <div className="flex-1">
                                <h3 className="font-bold text-base sm:text-lg text-gray-900 dark:text-white mb-2">{boss.name}</h3>
                                <div className="flex flex-wrap gap-2 mb-3">
                            ...
                            <button
                              onClick={(e) => copyCounters(boss.counters, e)}
                              className="w-full px-4 py-2 bg-gradient-to-r from-gray-600 to-slate-600 hover:from-gray-700 hover:to-slate-700 text-white rounded-lg font-semibold text-sm transition-all duration-200 hover:shadow-lg active:scale-95 flex items-center justify-center gap-2"
                            >
                              <Copy className="w-4 h-4" />
                              {getUIText('copy_counters', selectedLanguage)}
                            </button>
